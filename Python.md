...
2. `copy`浅拷贝，**没有拷贝当前对象内部的子对象**，所以当子对象的原始数据改变，拷贝后容器内的子对象也会改变。常见的在嵌套列表中，尽管对外层列表`a`的改变不会造成拷贝后的`b`的变化，但对`a`的内部子列表的改变会影响`b`。
...
> [!NOTE|label:参考资料]
> [python生成器和迭代器有这篇就够了](https://www.cnblogs.com/wj-1314/p/8490822.html)
>