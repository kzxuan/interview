...
def has_path(matrix, path):
    ...
    for row in range(rows):
        for col in range(cols):
            if has_path_core(matrix, row, col, path_len, path):
                return True

    del visited
    return False

def has_path_core(matrix, row, col, path_len, path):
    if path_len >= len(path):
        return True
    hasPath = False
    if 0 <= row < rows and 0 <= col < cols and matrix[row * cols + col] == path[path_len] and visited[row * cols + col] == 0:
        path_len += 1
        visited[row * cols + col] = 1
        hasPath = has_path_core(matrix, row + 1, col, path_len, path) or has_path_core(matrix, row - 1, col, path_len, path) or has_path_core(matrix, row, col + 1, path_len, path) or has_path_core(matrix, row, col - 1, path_len, path)
        if not hasPath:
            path_len -= 1
